...
generator = (mkSqlGenerator generator) { sqlType = postgresqlType
...
postgresqlUpdate name exprs =
  defaultSqlUpdate generator name exprs . map (first quote)

postgresqlTable :: TableName -> Scheme -> SqlSelect
postgresqlTable tablename scheme = 
  defaultSqlTable generator (quote tablename) (map quote scheme)
...
postgresqlInsert n =
    defaultSqlInsert generator (quote n) . map (first quote)
...
postgresqlLiteral (StringLit l) = "E" ++ def where
    def = defaultSqlLiteral generator (StringLit l)
